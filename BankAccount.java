...
public class BankAccount {
    ...
    public void checkOverdraft(BankAccount bank, double balance)
    {
        if (balance < 0)
        {
            System.out.println("Your account is getting charged an overdraft fee of $35.");
            withdraw(35);
        } else {
            balance -= amount;
        }
    ...
    public void moneyTransfer(BankAccount bank, double k)
    {
        if (k <= this.balance)
        {
            withdraw(k);
            bank.deposit(k);
            System.out.println("Transfer Done. Transfer: $" +k);
        } else {
    ...
    public void simpleInterest(double principal, int time, double rate, int annum){
        double sInterest = ((principal * rate * time)/100);
        System.out.println("Current Rate is: " +rate+"%");
        System.out.println("Current Period is: "+annum+" times per year");
        System.out.println("Selected Time is: "+time+" years");
        System.out.println("Simple Interest after " + time + " years: $" + sInterest);
    }
    ...
    public static void interact(BankAccount account){
        ...
        if (choice == 1) {
            ...
            double amount = input.nextDouble();
            account.checkOverdraft(account, choice);
            ...
            System.out.println("Would you like your interest be compounded interest or simple interested? 1 for compounded or 2 for simple");
            if(choice == 1){
                System.out.println("You have chosen compounded interest.");
                ...
                System.out.println("You have chosen simple interest.");
                account.simpleInterest(account.getBalance(), time, account.interestRatet, account.period);
            }
            System.out.println("... Thank you for using the Bank Account Menu. Have a nice day! ...");
        } else if (choice == 5) {
            System.out.println("How much money would you like to transfer?");
            double amount = input.nextDouble();
            account.moneyTransfer(account, amount);
        } else if (choice == 6){
            int choice3 = input.nextInt();
            System.out.println("Would you like to create a savings account? 1 FOR YES 2 FOR NO");
            if (choice3 == 1 && account.balance >= 10000){
                System.out.println("You have succesfully made a savings account.");
                System.out.println("Interested will be compounded, please input values for principal and time in years");
